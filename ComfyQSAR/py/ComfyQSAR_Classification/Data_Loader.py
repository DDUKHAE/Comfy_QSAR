...
class Data_Loader_Classification:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "positive_file_path": ("STRING", {"default": "positive.sdf"}), # Added default examples
                "negative_file_path": ("STRING", {"default": "negative.sdf"}),
            }
        }
    ...
    def load_data(self, positive_file_path, negative_file_path):
        send_progress("🚀 Starting Data Loading...", 0)
        ...
        try:
            ...
            # Check positive file
            send_progress(f"⏳ Checking positive file: {positive_file_path}", 10)
            ...
            send_progress("   Positive file format OK.", 15)

            # Check negative file
            send_progress(f"⏳ Checking negative file: {negative_file_path}", 20)
            ...
            send_progress("   Negative file format OK.", 25)
        ...
        def count_molecules(file_path, file_type_label):
            send_progress(f"⏳ Counting molecules in {file_type_label} file...", 30 if file_type_label == "positive" else 60)
            count = 0
            try:
                ...
                send_progress(f"   Found {count} molecules in {file_type_label} file.", 50 if file_type_label == "positive" else 80)
                return count
            except Exception as e:
        ...
        try:
            ...
            send_progress("🎉 Data loading complete!", 100)
...
class Standardization_Classification:
    ...
    def standardize_data(self, positive_path, negative_path):
        send_progress("🚀 Starting Standardization...", 0)
        ...
        def process_file(file_path, output_name, progress_start, progress_end):
            send_progress(f"⏳ Standardizing {output_name} file: {os.path.basename(file_path)}...", progress_start)
            output_file = ""
            filtered_count = 0
            try:
                ...
                if file_path.endswith('.sdf'):
                    suppl = Chem.SDMolSupplier(file_path, removeHs=True, strictParsing=False) # Use strictParsing=False
                ...
                send_progress(f"   Finished standardizing {output_name}. Kept {filtered_count} molecules.", progress_end)
        ...
        try:
            ...
            error_msg = str(e)
            send_progress(error_msg) # Send the detailed error message
            return {"ui": {"text": create_text_container(error_msg)}, "result": ("", "")}
...
class Load_and_Standardize_Classification:
    ...
    def INPUT_TYPES(cls):
         return {
            "required": {
                "positive_file_path": ("STRING", {"default": "positive.sdf"}),
                "negative_file_path": ("STRING", {"default": "negative.sdf"}),
            }
        }
    ...
    def load_and_standardize(self, positive_file_path, negative_file_path):
        send_progress("🚀 Starting Load & Standardization...", 0)
        METAL_IONS = { # Define METAL_IONS inside the method or make it a class attribute
            'Li', 'Be', 'Na', 'Mg', 'Al', 'K', 'Ca', 'Sc', 'Ti', 'V', 'Cr', 'Mn', 'Fe', 'Co', 'Ni', 'Cu', 'Zn',
            'Ga', 'Ge', 'Rb', 'Sr', 'Y', 'Zr', 'Nb', 'Mo', 'Tc', 'Ru', 'Rh', 'Pd', 'Ag', 'Cd', 'In', 'Sn',
            'Sb', 'Cs', 'Ba', 'La', 'Ce', 'Pr', 'Nd', 'Pm', 'Sm', 'Eu', 'Gd', 'Tb', 'Dy', 'Ho', 'Er', 'Tm',
        ...
        try:
            ...
            send_progress(f"⏳ Checking positive file: {positive_file_path}", 6)
            if not os.path.exists(positive_file_path):
                raise FileNotFoundError(f"Positive file not found: {positive_file_path}")
            if not (positive_file_path.endswith('.smi') or positive_file_path.endswith('.csv') or positive_file_path.endswith('.sdf')):
                raise ValueError("Unsupported positive file format. Use .smi, .csv, or .sdf.")
            send_progress("   Positive file format OK.", 7)

            send_progress(f"⏳ Checking negative file: {negative_file_path}", 8)
            if not os.path.exists(negative_file_path):
                raise FileNotFoundError(f"Negative file not found: {negative_file_path}")
            if not (negative_file_path.endswith('.smi') or negative_file_path.endswith('.csv') or negative_file_path.endswith('.sdf')):
                raise ValueError("Unsupported negative file format. Use .smi, .csv, or .sdf.")
            send_progress("   Negative file format OK.", 10)
            # --- End File Checks ---
        ...
        def process_file(file_path, output_name, progress_start, progress_end):
            send_progress(f"⏳ Loading & Standardizing {output_name} file: {os.path.basename(file_path)}...", progress_start)
            output_file = ""
            original_count = 0
            filtered_count = 0
            try:
                ...
                send_progress(f"   Finished {output_name}. Original: {original_count}, Kept: {filtered_count}.", progress_end)
