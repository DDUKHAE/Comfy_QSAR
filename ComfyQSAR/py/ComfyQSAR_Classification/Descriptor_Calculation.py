...
class Descriptor_Calculations_Classification:
    ...
    def calculate_and_merge_descriptors(self, positive_path, negative_path, descriptor_type, detect_aromaticity, remove_salt, standardize_nitro, use_file_name_as_molname, retain_order, threads, waiting_jobs, max_runtime, max_cpd_per_file, headless, log, advanced):
        ...
        current_progress = 0
        # 이제 공통 유틸리티 함수 사용
        send_progress("🚀 Starting descriptor calculation...", current_progress, node_id)

        # --- 1. 초기 설정 및 디렉토리 생성 ---
        output_dir = "QSAR/Descriptor_Calculation"
        os.makedirs(output_dir, exist_ok=True)
        current_progress = PREPARE_PCT
        send_progress(f"📂 Output directory prepared: {output_dir}", current_progress, node_id)
        ...
        def process_file(input_path, tag, label, progress_start, progress_total_allocation):
            ...
            def update_sub_progress(sub_step_progress, message):
                ...
                # 공통 유틸리티 함수 사용
                send_progress(message, new_progress, node_id)
            ...
            try:
                ...
                if input_path.endswith('.sdf'):
                    update_sub_progress(sub_progress + READ_CHECK_WEIGHT * 0.5, f"   Format: SDF. Using directly.", node_id)
                    mol_dir_for_padel = input_path
                elif input_path.endswith(('.csv', '.smi')):
                    update_sub_progress(sub_progress + READ_CHECK_WEIGHT * 0.2, f"   Format: {input_path.split('.')[-1]}. Reading SMILES...", node_id)
                    try:
                    ...
                    update_sub_progress(sub_progress + READ_CHECK_WEIGHT, f"   Created temporary SMILES file: {smiles_path}", node_id)
                    mol_dir_for_padel = smiles_path
                else: raise ValueError(f"Unsupported file format: {input_path}")
                sub_progress += READ_CHECK_WEIGHT

                # 2. PaDEL 실행
                update_sub_progress(sub_progress + PADEL_WEIGHT * 0.05, f"   Running PaDEL-Descriptor for {tag}...", node_id)
                padeldescriptor(mol_dir=mol_dir_for_padel, d_file=desc_file, **padel_options)
                sub_progress += PADEL_WEIGHT
                update_sub_progress(sub_progress, f"   PaDEL finished for {tag}. Output: {desc_file}", node_id)
                ...
                update_sub_progress(sub_progress, f"   Added 'Label' column ({label}) to {tag} descriptors.", node_id)
        ...
        try:
            ...
            send_progress(f"✅ Positive file processing complete. {len(df_positive)} molecules.", current_progress, node_id)
            ...
            send_progress(f"✅ Negative file processing complete. {len(df_negative)} molecules.", current_progress, node_id)

            # 결과 병합 및 저장
            merge_start_progress = current_progress
            merge_progress_allocation = 100 - merge_start_progress

            send_progress("🔗 Merging positive and negative sets...", merge_start_progress + merge_progress_allocation * 0.2, node_id)
            df_final = pd.concat([df_positive, df_negative], ignore_index=True)
            send_progress("💾 Saving final merged descriptors...", merge_start_progress + merge_progress_allocation * 0.6, node_id)
            final_file = os.path.join(output_dir, "final_merged_descriptors.csv")
            df_final.to_csv(final_file, index=False)
            current_progress = 98
            send_progress(f"📊 Total molecules processed: {len(df_final)}. File saved to: {final_file}", current_progress, node_id)
            ...
            send_progress("🎉 Calculation and merge finished successfully!", current_progress, node_id)
            ...
            send_progress(error_message, None, node_id) # 실패 시 progress 없이
