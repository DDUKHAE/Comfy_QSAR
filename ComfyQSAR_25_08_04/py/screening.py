...
class VirtualScreening:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                "model_path": ("STRING", {"placeholder": "ComfyUI/models/qsar_models/your_model.pkl", "tooltip": "Path to the trained QSAR model (.pkl)"}),
                "descriptors_path": ("STRING", {"placeholder": "ComfyUI/output/qsar_selection/selected_descriptors.txt", "tooltip": "Path to the selected descriptors list (.txt)"}),
                "selected_db": (db_names, {"placeholder": db_names[0] if db_names else "", "tooltip": "Select the screening database"}),
                "db_directory": ("STRING", {"placeholder": folder_paths.get_input_directory() + "/qsar_screening_db", "tooltip": "Directory containing the screening database folders (ASINEX, IBS_NP, etc.)"}),
                "task_type": (["Regression", "Classification"], {"placeholder": "Regression", "tooltip": "Type of QSAR task the model performs"}),
                "threshold": ("FLOAT", {"default": 10.0, "step": 0.1, "tooltip": "Threshold for selection (Regression: value <= threshold, Classification: prob >= threshold)"}),
            },
        }
    ...
    def screen(self, model_path, descriptors_path, selected_db, db_directory, task_type, threshold):
        ...
        try:
            ...
            import json
            encoded_summary = f"QSAR_SUMMARY:{json.dumps(summary_lines)}"
            ...
            return {"ui": {"text": encoded_summary, "text2": text_container_content},
            ...
            return {"ui": {"text": encoded_summary, "text2": text_container_content}, "result": ("", "")}
            ...
            import json
            encoded_summary = f"QSAR_SUMMARY:{json.dumps(summary_lines)}"
            text_container_content = create_text_container(encoded_summary)
            import traceback
    ...
    def extract(self, selected_db, db_directory, index_file):
        ...
        try:
            ...
            if num_requested == 0:
                 ...
                 encoded_summary = f"QSAR_SUMMARY:{json.dumps(summary_lines)}"
                 text_container_content = create_text_container(encoded_summary)
                 return {"ui": {"text": encoded_summary, "text2": text_container_content}, "result": (output_sdf_path,)}
...
# --- Combined Virtual Screening with Molecule Extraction Node ---
class VirtualScreeningWithExtraction:
    @classmethod
    def INPUT_TYPES(cls):
        # Get database names for the dropdown
        db_names = list(SCREENING_DATABASES.keys())
    ...
    RETURN_NAMES = ("SELECTED_COMPOUNDS_CSV", "SELECTED_INDICES_TXT", "EXTRACTED_SDF_PATH",)
    FUNCTION = "screen_and_extract"
    CATEGORY = "QSAR/Screening"
    OUTPUT_NODE = True

    def screen_and_extract(self, model_path, descriptors_path, selected_db, db_directory, task_type, threshold):
        node_id = os.environ.get("NODE_ID")
        send_progress(f"🚀 Starting Virtual Screening with Extraction (DB: {selected_db}, Task: {task_type})...", 0, node_id)
        ...
        try:
            # Check RDKit availability for molecule extraction
            if not RDKIT_AVAILABLE:
                raise ImportError("RDKit is not installed. This node requires RDKit for molecule extraction.")

            # Ensure output directory exists
            os.makedirs(output_node_dir, exist_ok=True)
            send_progress(f"📂 Output directory set: {output_node_dir}", 2, node_id)

            # --- PART 1: Virtual Screening (based on VirtualScreening node) ---
            ...
            db_info = SCREENING_DATABASES[selected_db]
            screening_csv_rel_path = db_info[0] # Get CSV relative path
            screening_sdf_rel_path = db_info[1] # Get SDF relative path
            screening_csv_abs_path = os.path.join(db_directory, screening_csv_rel_path)
            screening_sdf_abs_path = os.path.join(db_directory, screening_sdf_rel_path)

            if not os.path.exists(screening_csv_abs_path):
                 raise FileNotFoundError(f"Screening DB CSV file not found: {screening_csv_abs_path}")
            if not os.path.exists(screening_sdf_abs_path):
                 raise FileNotFoundError(f"Screening DB SDF file not found: {screening_sdf_abs_path}")
            send_progress("   Input paths validated.", 5, node_id)

            # Load Data and Model
            send_progress(f"⏳ Loading screening dataset: {os.path.basename(screening_csv_abs_path)}...", 10, node_id)
            screening_data = pd.read_csv(screening_csv_abs_path)
            original_indices = screening_data.index
            send_progress(f"   Screening data loaded ({screening_data.shape[0]} compounds, {screening_data.shape[1]} columns).", 15, node_id)

            send_progress(f"⏳ Loading QSAR model from {os.path.basename(model_path)}...", 20, node_id)
            model = joblib.load(model_path)
            send_progress("   Model loaded.", 25, node_id)

            send_progress(f"⏳ Loading selected descriptors from {os.path.basename(descriptors_path)}...", 30, node_id)
            with open(descriptors_path, "r") as f:
                selected_descriptors = [line.strip() for line in f if line.strip()]
            if not selected_descriptors:
                 raise ValueError("No descriptors found in the selected descriptors file.")
            send_progress(f"   Loaded {len(selected_descriptors)} selected descriptors.", 35, node_id)

            # Prepare Screening Data
            send_progress("⚙️ Preparing screening data (selecting features)...", 40, node_id)
            missing_descriptors = [desc for desc in selected_descriptors if desc not in screening_data.columns]
            if missing_descriptors:
                raise ValueError(f"Missing descriptors in screening dataset '{selected_db}': {missing_descriptors}")

            X_screen = screening_data[selected_descriptors]
            # Handle potential NaN/Inf in screening data
            X_screen = X_screen.replace([np.inf, -np.inf], np.nan)
            if X_screen.isnull().values.any():
                  send_progress("   Imputing NaN/Inf values in screening data (median)...", 42, node_id)
                  imputer = SimpleImputer(strategy='median')
                  X_screen_imputed = imputer.fit_transform(X_screen)
                  X_screen = pd.DataFrame(X_screen_imputed, columns=selected_descriptors, index=X_screen.index)
            send_progress("   Screening data prepared.", 45, node_id)

            # Perform Predictions
            send_progress("🤖 Performing predictions...", 50, node_id)
            predictions = None
            selected_df_indices = np.array([], dtype=int)

            if task_type == "Classification":
                if hasattr(model, "predict_proba"):
                    predictions = model.predict_proba(X_screen)[:, 1]
                    selected_df_indices = np.where(predictions >= threshold)[0]
                    send_progress(f"   Classification predictions (probability) completed. Threshold >= {threshold}", 55, node_id)
                else:
                    predictions = model.predict(X_screen)
                    try: predicted_class = int(threshold)
                    except ValueError: raise ValueError("Threshold for classification 'predict' must be an integer class label (e.g., 1).")
                    selected_df_indices = np.where(predictions == predicted_class)[0]
                    send_progress(f"   Classification predictions (label) completed. Selected class = {predicted_class}", 55, node_id)

            elif task_type == "Regression":
                if hasattr(model, "predict"):
                    predictions = model.predict(X_screen)
                    selected_df_indices = np.where(predictions <= threshold)[0]
                    send_progress(f"   Regression predictions completed. Threshold <= {threshold}", 55, node_id)
                else:
                    raise AttributeError(f"The loaded model (type: {type(model)}) does not have a 'predict' method required for regression.")
            else:
                 raise ValueError(f"Invalid task_type: {task_type}. Choose 'Regression' or 'Classification'.")
            ...
            num_selected = len(selected_df_indices)
            send_progress(f"   Found {num_selected} compounds meeting the threshold criteria.", 60, node_id)

            # Save Virtual Screening Results
            send_progress("💾 Saving virtual screening results...", 65, node_id)
            ...
            selected_indices_path = os.path.join(output_node_dir, indices_filename)
            np.savetxt(selected_indices_path, selected_original_indices, fmt="%d")
            send_progress(f"   Selected original indices saved: {indices_filename}", 67, node_id)
            ...
            if num_selected > 0:
                 ...
                 selected_compounds_df_path = os.path.join(output_node_dir, compounds_filename)
                 selected_compounds_df.to_csv(selected_compounds_df_path, index=False)
                 send_progress(f"   Selected compounds data saved: {compounds_filename}", 70, node_id)
            else:
                 selected_compounds_df_path = "N/A (No compounds selected)"
                 send_progress("   No compounds selected, skipping saving compounds CSV.", 70, node_id)

            # --- PART 2: Molecule Extraction (based on ExtractScreenedMolecules node) ---
            
            if num_selected > 0:
                send_progress(f"🧬 Extracting {num_selected} molecules from {os.path.basename(screening_sdf_abs_path)}...", 75, node_id)
                
                # Check if SDF file is valid and readable by RDKit
                try:
                    sdf_supplier = Chem.SDMolSupplier(screening_sdf_abs_path)
                    if not sdf_supplier:
                        raise ValueError(f"Could not open or parse SDF file: {screening_sdf_abs_path}")
                    sdf_len = len(sdf_supplier)
                    if sdf_len == 0:
                        raise ValueError(f"SDF file is empty: {screening_sdf_abs_path}")
                except Exception as e:
                     raise ValueError(f"Error opening/parsing SDF file '{screening_sdf_abs_path}' with RDKit: {e}")

                output_sdf_filename = f"Extracted_{selected_db}_{num_selected}_molecules_{timestamp}.sdf"
                output_sdf_path = os.path.join(output_node_dir, output_sdf_filename)

                sdf_writer = Chem.SDWriter(output_sdf_path)

                # Convert indices to a set for efficient lookup
                indices_set = set(selected_original_indices)
                max_index_requested = max(indices_set) if indices_set else -1
                ...
                send_progress(f"   Iterating through {sdf_len} records in SDF...", 77, node_id)

                for current_idx, mol in enumerate(sdf_supplier):
                      # Stop if we've processed beyond the highest requested index
                      if current_idx > max_index_requested and not indices_set:
                          send_progress(f"   Highest requested index {max_index_requested} processed. Stopping early.", 90, node_id)
                          break
                      ...
                      processed_count += 1
                      if processed_count % update_interval == 0:
                           progress = 77 + int(13 * (processed_count / sdf_len))
                           send_progress(f"   Processed {processed_count}/{sdf_len} records...", min(progress, 90), node_id)

                      # Stop if all requested indices are found
                      if not indices_set:
                           send_progress(f"   All {num_selected} requested indices found and processed.", 90, node_id)
                           break

                sdf_writer.close()
                send_progress(f"   Extraction complete. Extracted {num_extracted} valid molecules.", 92, node_id)

                if indices_set:
                     print(f"[ComfyQSAR Screening] Warning: Could not find records for the following indices: {sorted(list(indices_set))}")

            else:
                output_sdf_path = "N/A (No compounds selected)"
                send_progress("   No compounds selected, skipping molecule extraction.", 92, node_id)

            # --- Generate Final Summary ---
            send_progress("📝 Generating final summary...", 95, node_id)
            summary_lines = [
                "**Virtual Screening with Extraction Completed!**",
                f"Database: {selected_db}",
                f"Model: {os.path.basename(model_path)}",
                f"Task: {task_type}",
                f"Criteria: {'<=' if task_type=='Regression' else '>='} {threshold}",
                ...
                f"Selected Compounds CSV: {os.path.basename(selected_compounds_df_path) if num_selected > 0 else 'N/A'}",
                f"Selected Indices TXT: {os.path.basename(selected_indices_path)}",
                f"Extracted Molecules SDF: {os.path.basename(output_sdf_path) if num_selected > 0 else 'N/A'}"
            ]
            import json
            encoded_summary = f"QSAR_SUMMARY:{json.dumps(summary_lines)}"
            text_container_content = create_text_container(encoded_summary)
            send_progress("🎉 Virtual screening with extraction process finished successfully!", 100, node_id)

            # Return the absolute paths to the created files
            return {"ui": {"text": encoded_summary, "text2": text_container_content},
                    "result": (str(selected_compounds_df_path), str(selected_indices_path), str(output_sdf_path),)}
            ...
            error_msg = f"❌ An unexpected error occurred during screening and extraction: {str(e)}"
            import json
            encoded_summary = f"QSAR_SUMMARY:{json.dumps(summary_lines)}"
            text_container_content = create_text_container(encoded_summary)
            import traceback
    ...
    def screen_custom(self, model_path, descriptors_path, input_compound_file, input_type, task_type,
        ...
        try:
            ...
            return {"ui": {"text": encoded_summary, "text2": text_container_content},
             ...
             return {"ui": {"text": encoded_summary, "text2": text_container_content}, "result": ("", "")} # Return empty strings for paths
        except ValueError as ve:
...
# Conditionally add RDKit dependent nodes
if RDKIT_AVAILABLE:
     ...
     # Add the combined Virtual Screening with Extraction node
     NODE_CLASS_MAPPINGS["VirtualScreeningWithExtraction"] = VirtualScreeningWithExtraction
     NODE_DISPLAY_NAME_MAPPINGS["VirtualScreeningWithExtraction"] = "Virtual Screening + Extraction"
else:
     print("[ComfyQSAR Screening] ExtractScreenedMolecules and VirtualScreeningWithExtraction nodes registration skipped (RDKit not found).")
