...
try:
    from .Data_Loader import send_progress, create_text_container, create_qsar_pages
except ImportError:
    ...
    def create_qsar_pages(title, pages_data=None, simple_text=None):
        ...
        if simple_text is not None:
            ...
            return f"QSAR_TITLE:{json.dumps(title)}", f"QSAR_PAGES:{json.dumps(error_pages)}"
        elif pages_data is not None:
            return f"QSAR_TITLE:{json.dumps(title)}", f"QSAR_PAGES:{json.dumps(pages_data)}"
        else:
            error_pages = {"error": "No data provided"}
            return f"QSAR_TITLE:{json.dumps('Error')}", f"QSAR_PAGES:{json.dumps(error_pages)}"
...
class Regression_Feature_Combination_Search:
    ...
    def find_best_combinations(self, input_csv_path, evaluation_model, max_features, num_cores, top_n):
        ...
        try:
            ...
            pages_title = "🔹 Regression Feature Combination Search Completed! 🔹"
            pages_data = {
                "summary": f"Input File: {os.path.basename(input_csv_path)}\nEvaluation Model: {evaluation_model}\n✅ Best R² Score: {best_result['R2_Score']:.4f} (with {len(best_features_list_overall)} features)\n   - MSE{'(scaled)' if evaluation_model=='LinearRegression' else ''}: {best_result['MSE']:.4f}\n   - Features: {', '.join(best_features_list_overall)}\n💾 Best Set File: {output_file_path}\n💾 Per-Size Best Combinations: {best_per_size_path}\n💾 Top {len(optimal_feature_paths)} Sets Saved in: {output_dir}\n🔍 Total Combinations Evaluated: {total_combinations_evaluated} (Valid Results: {total_valid_results})"
            }
            
            title, pages = create_qsar_pages(pages_title, pages_data)
            send_progress("🎉 Feature combination search finished successfully!", 100)

            return {"ui": {"text": title, "text2": pages},
                    "result": (str(output_file_path),)} # Return path to the best (rank 1) set

        except FileNotFoundError as fnf_e:
            error_msg = f"❌ File Not Found Error: {str(fnf_e)}."
            send_progress(error_msg)
            title, pages = create_qsar_pages("❌ Feature Combination Error", simple_text=error_msg)
